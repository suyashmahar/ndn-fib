...
foreach file $fileList {
    puts "\n=============\nNow generating ip for $file"
    set fileLength [lindex "${sizeList}" $fileCounter]
    # Set BRAM depth to 2 or greater than 2
    set bramDepth [expr $fileLength == 1 ? 2 : $fileLength]
    ...
    set_property -dict [list CONFIG.Memory_Type {Dual_Port_ROM} CONFIG.Write_Width_A "$strideSize" CONFIG.Write_Depth_A "$bramDepth" CONFIG.Read_Width_A "$strideSize" CONFIG.Enable_A {Always_Enabled} CONFIG.Write_Width_B "$strideSize" CONFIG.Read_Width_B "$strideSize" CONFIG.Enable_B {Always_Enabled} CONFIG.Register_PortA_Output_of_Memory_Primitives {false} CONFIG.Register_PortB_Output_of_Memory_Primitives {false} CONFIG.Load_Init_File {true} CONFIG.Coe_File "$file" CONFIG.Port_A_Write_Rate {0} CONFIG.Port_B_Clock {100} CONFIG.Port_B_Enable_Rate {100} ] [ get_ips "${ipName}" ]
